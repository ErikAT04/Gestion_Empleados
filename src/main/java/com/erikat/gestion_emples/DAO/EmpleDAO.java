...
public class EmpleDAO {
    ...
    public int modEmple(Emple emple){ //Recibe un empleado entero
    ...
    public Emple searchEmple(String dni){ //Busca un empleado por su DNI
        ...
        try {
            ...
            while (rs.next()){ //Si encuentra un empleado:
                DepartDAO dptFinder = new DepartDAO(); //Crea un objeto de tipo DepartDAO
                String name = rs.getString("emp_name");
                String surname = rs.getString("emp_surname");
                double salary = rs.getDouble("emp_salary");
                ...
                Depart dpt = dptFinder.searchDept(rs.getInt("working_dept")); //Busca el departamento por medio del código que tiene guardado en su tabla de MySQL
